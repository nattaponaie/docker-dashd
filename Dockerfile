...
RUN apk --no-cache add \
  ...
  p7zip \
  git
RUN (git clone -b v0.12.1.x --depth 10 https://github.com/dashpay/dash.git) \
  && cd dash \
  && ./autogen.sh \
...
RUN apk del libtool autoconf automake alpine-sdk wget git
RUN ldd /usr/local/bin/dashd
RUN rm -rf /dash
